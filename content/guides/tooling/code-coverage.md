...
the back end coverage, add the new endpoint to the
[configuration file](/guides/references/configuration) environment settings
under `env.codeCoverage.url` key. For example, if the application back end is
running at port 3000 and we are using the default "GET /**coverage**" endpoint,
set the following:
